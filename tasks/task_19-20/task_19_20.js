...
// 20 Реализовать функцию подсчета объема памяти занимаемого данными в LocalStorage для предыдущей задачи. При изменении данных в localStorage
// в консоль должен выводиться объем занятой памяти / максимальный размер хранилища.

// !!! открываем страницу
// https://oauth.vk.com/authorize?client_id=51783130&display=page&redirect_uri=http://127.0.0.1:5500/index.html&scope=offline&response_type=token

// ведет пользователя на авторизацию через VK и последующий редирект на http://127.0.0.1:5500/index.html
...
maxSpaceLocalStorage = testLocalStorageLimit();
...
const caltulateSizeItemsInLocalStorage = () => {
  ...
  console.log(`Размер localStorage: ${maxSpaceLocalStorage} килобайт`);
};
...
const fetchPosts = () =>
  ...
  VK.Api.call(
    "wall.get",
    {
      owner_id: -11982368, // id сообщества VK - Forbes
      domain: "forbes", // Короткий адрес пользователя или сообщества.
...
const createItem = (data) => {
  ...
  date.innerText = `${dateData.toLocaleDateString()} ${dateData.toLocaleTimeString(
    [],
